...
/** Assembles the rating input from the earlier stages' collected state. */
export function buildRatingSubject(collected: {
  driver?: DriverProfile | undefined;
  use?: VehicleUse | undefined;
  history?: InsuranceHistory | undefined;
}): { subject: RatingSubject } | { error: string } {
  const { driver, use, history } = collected;
