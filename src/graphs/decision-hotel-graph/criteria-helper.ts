...
export class CriteriaHelper {
  private static criteriaNodes: Readonly<
    Record<CriteriaField, GraphNodeTarget>
  > | null = null;
  ...
  static readCriteria(
    ...
    return {
      dates: {
        ...
        start: this.stringOrNull(dates.start),
        end: this.stringOrNull(dates.end),
      },
      budget: {
        answered: budget.answered === true,
        min: this.finiteOrNull(budget.min),
        max: this.finiteOrNull(budget.max),
      },
      roomType: {
        answered: room.answered === true,
        roomType: this.ROOM_TYPES.includes(room.roomType as RoomType)
      ...
      amenities: {
        ...
        amenities: Array.isArray(amenities.amenities)
          ? amenities.amenities.filter((value): value is Amenity =>
            this.AMENITIES.includes(value as Amenity),
          )
          : [],
      },
      distance: {
        answered: distance.answered === true,
        airport: this.finiteOrNull(distance.airport),
        cityCenter: this.finiteOrNull(distance.cityCenter),
      },
    };
  }
  ...
  static validateCriteria(criteria: HotelCriteriaSnapshot): CriteriaIssue[] {
    ...
    if (!criteria.dates.answered)
      issues.push({
        field: "dates",
        message: "Stay dates have not been answered.",
      });
    else {
      ...
      else if (start <= this.currentBusinessDate())
        issues.push({
          field: "dates",
    ...
    if (!criteria.budget.answered)
      issues.push({
        field: "budget",
        message: "Nightly budget has not been answered.",
      });
    else if (
      ...
      issues.push({
        field: "budget",
        message:
          "Budget values must be nonnegative and minimum cannot exceed maximum.",
      });
    if (!criteria.roomType.answered || !criteria.roomType.roomType)
      issues.push({
        field: "room_type",
        message: "Room type has not been answered.",
      });
    if (!criteria.amenities.answered)
      issues.push({
        field: "amenities",
        message: "Amenity preferences have not been answered.",
      });
    if (!criteria.distance.answered)
      issues.push({
        field: "distance",
        message: "Distance preferences have not been answered.",
      });
    else if (
      (criteria.distance.airport !== null && criteria.distance.airport < 0) ||
      (criteria.distance.cityCenter !== null &&
        criteria.distance.cityCenter < 0)
    )
      issues.push({
        field: "distance",
  ...
  static currentBusinessDate(): Date {
    const date = new Date(
      process.env.HOTEL_GRAPH_CURRENT_DATE ?? new Date().toISOString(),
    );
  ...
  static registerCriteriaNodes(
    nodes: Readonly<Record<CriteriaField, GraphNodeTarget>>,
  ): void {
  ...
  static nextNode(
    fieldOrIssue: CriteriaField | CriteriaIssue,
  ): GraphNodeTarget {
    const field =
      typeof fieldOrIssue === "string" ? fieldOrIssue : fieldOrIssue.field;
    const node = this.criteriaNodes?.[field];
    if (!node)
      throw new Error(`Criteria node is not registered for field: ${field}`);
    return node;
  }
  ...
  static criteriaPrompt(field: CriteriaField): string {
    switch (field) {
      case "dates":
        return "What are your Portland check-in and checkout dates?";
      case "budget":
        return "What nightly budget range should I use?";
      case "room_type":
        return "Would you like one bed, two beds, or a suite?";
      case "amenities":
        return "Which hotel amenities do you require?";
      case "distance":
        return "Do you have an airport or city-center distance limit?";
    }
  }

  static criterionAnswered(
    criteria: HotelCriteriaSnapshot,
    field: CriteriaField,
  ): boolean {
    switch (field) {
      case "dates":
        return criteria.dates.answered;
      case "budget":
        return criteria.budget.answered;
      case "room_type":
        return criteria.roomType.answered;
      case "amenities":
        return criteria.amenities.answered;
      case "distance":
        return criteria.distance.answered;
    }
  }
