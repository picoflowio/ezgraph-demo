...
export class DecisionHotelGraph extends BaseGraph<DecisionHotelGraphStateType> {
  static getGraphDefinition(): GraphDefinition {
    return {
      ...
      decisionConfig: { provider: 'typesafe', model: 'jev-latest', timeoutMs: 15_000, maxRetries: 2 },
      endNode: GRAPH_END_NODE,
      llmTimeoutMs: 60_000,
      initialHistorySpace: 'hotel-intake',
      historySpaces: [
        [RouterDecisionNode, 'hotel-intake'], [DateRangeNode, 'hotel-intake'], [BudgetNode, 'hotel-intake'],
        [RoomTypeNode, 'hotel-intake'], [AmenityNode, 'hotel-intake'], [DistanceNode, 'hotel-intake'],
        [CriteriaReadinessDecisionNode, 'hotel-intake'], [PresentNode, 'hotel-present'],
        [PresentationDecisionNode, 'hotel-present'], [TerminateSessionNode, 'hotel-terminal'],
      ],
    };
  }
  constructor(gateway: LlmGateway) { super(gateway, DecisionHotelGraph.getGraphDefinition()); }
  protected override onDecisionError(context: DecisionErrorContext<DecisionHotelGraphStateType>): GraphNodeResponse<DecisionHotelGraphStateType> {
    if (context.nodeId === RouterDecisionNode.id()) return routerFallback(context.state);
    if (context.nodeId === CriteriaReadinessDecisionNode.id()) return criteriaFallback(context.state);
    if (context.nodeId === PresentationDecisionNode.id()) return presentationFallback(context.state);
    throw context.error;
  }
  protected buildGraph() {
    const graph = this.createStateGraph(DecisionHotelGraphState);
    graph.registerTurnNodes(
      RouterDecisionNode, DateRangeNode, BudgetNode, RoomTypeNode, AmenityNode, DistanceNode,
      CriteriaReadinessDecisionNode, SearchHotelsNode, PresentNode, PresentationDecisionNode, TerminateSessionNode,
    );
