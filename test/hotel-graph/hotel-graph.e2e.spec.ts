...
const scenarioPath = join(
  ...
  "test",
  "hotel-graph",
  "hotel-graph.scenario.json",
);
const failureArtifactPath = join(
  process.cwd(),
  "test",
  ".tmp",
  "hotel-graph-semantic-failure.json",
);
...
const judgeModel =
  process.env.HOTEL_GRAPH_JUDGE_MODEL ?? scenario.judgeModel ?? "gpt-4o";
const testTimeoutMs = Number(
  process.env.HOTEL_GRAPH_TEST_TIMEOUT_MS ?? 900_000,
);
const missingLiveConfig = ["OPENAI_API_KEY"].filter(
  (key) => !process.env[key]?.trim(),
);
const shouldRunLiveTest =
  process.env.RUN_LIVE_HOTEL_GRAPH_TEST !== "0" &&
  missingLiveConfig.length === 0;
const skipReason =
  process.env.RUN_LIVE_HOTEL_GRAPH_TEST === "0"
    ? "RUN_LIVE_HOTEL_GRAPH_TEST=0"
    : `Missing live HotelGraph config: ${missingLiveConfig.join(", ")}`;
const useEnvironmentSessionStore =
  process.env.HOTEL_GRAPH_TEST_USE_ENV === "1";
const keepSession = process.env.HOTEL_GRAPH_KEEP_SESSIONS === "1";

test(
  "HotelGraph completes a realistic search, comparison, and booking conversation",
  ...
  async () => {
    ...
    try {
      ...
      if (sessionId && keepSession) {
        logProgress(`retained MongoDB session: ${sessionId}`);
      } else if (sessionId) {
...
async function createApp(): Promise<NestFastifyApplication> {
  ...
  if (!useEnvironmentSessionStore) {
    moduleBuilder
      ...
      .useValue(new GraphEngine({ graphs: [HotelGraph] }));
  }
...
function logProgress(message: string): void {
  if (process.env.HOTEL_GRAPH_TEST_LOG === "0") return;
  console.log(`[HotelGraph E2E] ${message}`);
}
...
function loadScenario(): HotelGraphScenario {
  const parsed = JSON.parse(
    readFileSync(scenarioPath, "utf8"),
  ) as HotelGraphScenario;

  assert.equal(parsed.graphName, "HotelGraph");
        ...
        messages: [
          {
            role: "system",
            content: [
              "You are a strict but fair evaluator for an AI hotel booking graph test.",
...
async function expectSessionState(
  ...
  const session = await app
    .get(GraphEngine)
    .getSession<HotelGraphStateType>(sessionId);
  assert.ok(session, "Expected HotelGraph session document");
  assert.equal(session.graph.id, "HotelGraph");
  assert.equal(session.status, "completed");
  assert.equal(session.graph.currentNode, "end");
  assert.ok(session.graph.nodes, "Expected HotelGraph node state");

  const criteria = session.graph.nodes.ExploreNode?.criteria;
  assert.ok(criteria, "Expected ExploreNode search criteria");
  assert.equal(criteria.cDate.start, "2027-08-01");
  assert.equal(criteria.cDate.end, "2027-08-08");
  assert.deepEqual(criteria.cRoomType, ["two beds"]);
  assert.deepEqual(criteria.cAmenities, ["freeWiFi", "freeParking"]);
  assert.equal(criteria.cPriceRange.max, 700);

  const hotelFound = session.graph.nodes.PresentNode?.hotelFound;
  ...
  assert.equal(
    session.graph.nodes.PresentNode?.hotel,
    "Hampton Inn Sherwood Portland",
  );
  assert.deepEqual(session.graph.nodes.CompareNode?.selectedHotels, [
